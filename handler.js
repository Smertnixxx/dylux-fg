        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (!isPrems && plugin.diamond && global.db.data.users[m.sender].diamond < plugin.diamond * 1) {
                    this.reply(m.chat, `✳️ Твои бриллианты закончились\используйте следующую команду, чтобы купить больше бриллиантов \n*${usedPrefix}Купить* <количество> \n*${usedPrefix}Купитьвсе*`, m)
                ...
                try {
                    ...
                    if (m.diamond)
                        m.reply(`Потраченно на испольование команды - *${+m.diamond}* 💎`)
                }
                break
            }
        }
        ...
        case 'remove':
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    let pp = 'https://ibb.co/TrSM4m9'
                    let ppgp = 'https://ibb.co/TrSM4m9'
                    try {
                        ...
                        text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Добро Пожаловать, @user').replace('@group', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || 'Неизвестный') :
                            (chat.sBye || this.bye || conn.bye || 'Покинул(а) Группу😓, @user')).replace('@user', '@' + user.split('@')[0])
            ...
            break
        case 'promote':
            text = (chat.sPromote || this.spromote || conn.spromote || '@user теперь он администратор')
        case 'demote':
            let pp = await this.profilePictureUrl(participants[0], 'image').catch(_ => 'https://ibb.co/TrSM4m9') 
            if (!text)
...
export async function groupsUpdate(groupsUpdate) {
    ...
    for (const groupUpdate of groupsUpdate) {
        ...
        if (groupUpdate.subject) text = (chats.sSubject || this.sSubject || conn.sSubject || 'Название группы было изменено на \n@group').replace('@group', groupUpdate.subject)
        if (groupUpdate.icon) text = (chats.sIcon || this.sIcon || conn.sIcon || 'Значок группы изменился на').replace('@icon', groupUpdate.icon)
        if (groupUpdate.revoke) text = (chats.sRevoke || this.sRevoke || conn.sRevoke || 'Ссылка на группу меняется на\n@revoke').replace('@revoke', groupUpdate.revoke)
        if (!text) continue
...
global.dfail = (type, m, conn) => {
    let msg = {
        ...
        mods: '🔰  Эта функция предназначена только для *Для модераторов ботов*',
        premium: '💠 Эта команда предназначена только для участников *Premium*\n\Ппишите */premium* для получения дополнительной информации',
        ...
        unreg: '📇 Зарегистрируйтесь, чтобы использовать эту функцию, набрав:\n\n* /reg имя.возраст*\n\N📌Пример : */reg OPENAI.16*',
