    ...
    let sock = Object.defineProperties(conn, {
        ...
        getFile: {
            ...
            async value(PATH, saveToFile = false) {
                ...
                if (!Buffer.isBuffer(data)) throw new TypeError('Результат не является буфером')
        ...
        waitEvent: {
            ...
            value(eventName, is = () => true, maxTries = 25) { //Idk why this exist?
                return new Promise((resolve, reject) => {
                    let tries = 0
                    let on = (...args) => {
                        if (++tries > maxTries) reject('Достигнуты максимальные попытки')
                        else if (is()) {
        ...
        sendFile: {
            ...
            async value(jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) {
                ...
                if (fileSize >= 1800) throw new Error(' ✳️  Размер файла слишком велик\n\n')
                let opt = {}
    ...
    */
    sendPoll: {
        ...
        sendGroupV4Invite: {
             ...
             * @param {*} options 
             */
            async value(jid, participant, inviteCode, inviteExpiration, groupName = 'unknown subject', caption = 'Приглашение присоединиться к моей группе в WhatsApp', jpegThumbnail, options = {}) {
...
export function logic(check, inp, out) {
    if (inp.length !== out.length) throw new Error('Входные и выходные данные должны иметь одинаковую длину')
