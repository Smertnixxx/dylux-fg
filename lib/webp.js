...
function _classPrivateMethodGet(receiver, privateSet, fn) {
  if (!privateSet.has(receiver)) {
    throw new TypeError("попытался получить приватное поле в не-экземпляре");
  }
  return fn;
}
...
class Image {
  ...
  demuxAnim(path, frame = -1, prefix = "#FNAME#") {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (end == 0) {
        throw new Error("Этот веб-сайт не является анимацией");
      }
    ...
    return _asyncToGenerator(function* () {
      ...
      switch (r.type) {
        ...
        case constants.TYPE_EXTENDED:
          if (r.hasAnim) {
            throw new Error("Объединение анимаций в настоящее время не поддерживается");
          }

          break;

        default:
          throw new Error("Неизвестный тип WebP");
      }
